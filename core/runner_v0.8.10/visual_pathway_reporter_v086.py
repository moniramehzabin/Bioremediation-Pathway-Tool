...
def main():
    ...
    for pid in pids:
        ...
        functional_note=''
        arsenic_split_html=''
        arsenic_rids={d.get('reaction_id','') for d in defs}
        if 'BRXN_ARS_C' in arsenic_rids and 'BRXN_ARS_EFFLUX' in arsenic_rids:
            def mini_module(rid,label,chemistry):
                rr=rev.get(rid,{})
                q,qlabel=quality(rr)
                state_r=rr.get('integrated_state','NO_CANDIDATE')
                supported=rr.get('supported_loci') or ''
                review_loci=rr.get('review_loci') or ''
                candidate=rr.get('candidate_only_loci') or ''
                counted = state_r.startswith('SUPPORTED') and q not in {'review','candidate','missing'}
                result = 'SUPPORTED' if counted else ('REVIEW' if q=='review' else ('CANDIDATE ONLY' if q=='candidate' else 'NOT DETECTED'))
                loci = supported or review_loci or candidate or '—'
                return f'''<div class="arsenic-mini {q}">
                  <div class="mini-title">{esc(label)}</div>
                  <div class="mini-chem">{esc(chemistry)}</div>
                  <div class="mini-result">{esc(result)}</div>
                  <div class="mini-evidence">{esc(qlabel)} · {esc(loci)}</div>
                </div>'''
            arsenic_split_html = '<div class="arsenic-split">' + \
                mini_module('BRXN_ARS_C','Arsenate reduction','As(V) to As(III)') + \
                mini_module('BRXN_ARS_EFFLUX','Arsenic resistance / efflux','Arsenite export / detoxification') + \
                '</div>'
            functional_note='<div class="functionnote"><b>Separated interpretation:</b> arsenate reduction and arsenic resistance/efflux are distinct biological capabilities. Efflux is not counted as evidence of arsenate reduction. The two cards below are scored from their own reaction evidence.</div>'
        context=''
...
<div class="head"><div><div class="pid">{esc(pid)}</div><h2>{esc(pname)}</h2></div><div class="badges"><span>{esc(STATE_LABEL.get(state,state))}</span><span>{esc(pathway_conf(evidence_rows))}</span></div></div>{functional_note}{arsenic_split_html}
...
<details><summary>Reaction & locus evidence</summary><div class="tablewrap"><table><thead><tr><th>Reaction</th><th>State</th><th>Supported loci</th><th>Review loci</th><th>Candidate-only</th><th>Evidence relationship</th></tr></thead><tbody>{''.join(trs) if trs else '<tr><td colspan="6">No reaction rows.</td></tr>'}</tbody></table></div></details></section>''')
    ...
    print('Visual Pathway Reporter v0.8.6')
