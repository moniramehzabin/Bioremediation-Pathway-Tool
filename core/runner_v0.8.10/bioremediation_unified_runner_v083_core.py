#!/usr/bin/env python3
"""
Bioremediation Unified Runner v0.8.3
...
def report(prefix,pathway,reaction,decisions):
    ...
    with open(fp,"w",encoding="utf-8") as f:
        f.write("BIOREMEDIATION PATHWAY REPORT — v0.8.4\n")
        f.write("="*76+"\n\n")
...
def main():
    ...
    ap.add_argument("--rescue-engine",default=str(Path(__file__).resolve().parent/"Bioremediation_Hybrid_Rescue_v0.3.5_resolvedskip"/"hybrid_rescue_engine_v0.3.5_resolvedskip.py"))
    ap.add_argument("--integrated-engine",default="Bioremediation_Integrated_Evidence_v0.6.3\\integrated_evidence_v063.py")
    ap.add_argument("--adjudicator",default=str(Path(__file__).resolve().parent/"cross_reaction_adjudicator_v088.py"))
    ap.add_argument("--policy",default=str(Path(__file__).resolve().parent/"Bioremediation_Hybrid_Rescue_v0.3.5_resolvedskip"/"reaction_interpro_policy_v054.tsv"))
    ...
    resolver=Path(__file__).resolve().parent/"resolved_annotation_scanner_v0810.py"
    run([a.python,str(resolver),str(gb),"--db",str(db),"--out",str(resolved)],log)
    ...
    visual_reporter=Path(__file__).resolve().parent/"visual_pathway_reporter_v086.py"
    run([a.python,str(visual_reporter),"--db",str(db),"--pathway-summary",str(aps),"--reaction-evidence",str(are),"--cross-reaction-decisions",str(dec),"--out-html",str(visual),"--sample-name",Path(a.genbank).stem],log)
    ...
    manifest={
        ...
        "outputs":{
            ...
            "report":str(rep),"visual_report":str(visual),"log":log
        }
    }
    ...
    print("\n"+"="*76)
    print("UNIFIED RUN v0.8.8 COMPLETE")
    print("="*76)
