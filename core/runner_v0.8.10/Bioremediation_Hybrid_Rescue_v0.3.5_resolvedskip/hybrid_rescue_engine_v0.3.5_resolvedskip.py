...
def main():
    ...
    # Propagate component metadata conservatively from reaction_components.tsv
    # into reference metadata when older curated rows predate the explicit
    # component_id/component_gene_family columns.  This does NOT infer new
    # biology: a reference is assigned only when exactly one required component
    # matches an existing target_id or an exact curated gene-family token.
    def _tokens(value):
        import re
        return {t.lower() for t in re.split(r"[;,/|\s]+", str(value or "")) if t.strip()}

    component_defs = defaultdict(list)
    for cr in component_rows:
        rid = str(cr.get("reaction_id", "")).strip()
        cid = str(cr.get("component_id", "")).strip()
        if rid and cid:
            component_defs[rid].append(cr)

    component_map_audit = []
    for ref_id, meta in ref_meta.items():
        if str(meta.get("component_id", "")).strip():
            continue
        rid = str(meta.get("reaction_id", "")).strip()
        defs = component_defs.get(rid, [])
        if not defs:
            continue
        source_target = str(meta.get("source_target_id", "")).strip()
        gene_tokens = _tokens(meta.get("gene_primary", ""))
        matches = []
        for cr in defs:
            target_id = str(cr.get("target_id", "")).strip()
            gf_tokens = _tokens(cr.get("gene_family", ""))
            target_match = bool(source_target and target_id and source_target == target_id)
            gene_match = bool(gene_tokens and gf_tokens and (gene_tokens & gf_tokens))
            if target_match or gene_match:
                matches.append(cr)
        # Never guess when metadata is ambiguous.
        unique = {str(m.get("component_id", "")).strip(): m for m in matches if str(m.get("component_id", "")).strip()}
        if len(unique) == 1:
            cid, cr = next(iter(unique.items()))
            meta["component_id"] = cid
            meta["component_gene_family"] = str(cr.get("gene_family", "")).strip()
            component_map_audit.append((ref_id, rid, cid, meta["component_gene_family"]))

    if component_map_audit:
        print(f"Component metadata propagated for {len(component_map_audit)} curated reference(s).")
