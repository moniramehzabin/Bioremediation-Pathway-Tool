...
def rx_state(r):
    ...
    if state in {"NO_CANDIDATE","NOT_DETECTED",""} and not truth(r.get("candidate_present")):
        return "NOT_DETECTED"
    if review or state in {"CANDIDATE_CONFLICTING","SUPPORTED_MIXED_LOCI"} or "MIXED" in relationship:
        return "REVIEW"
    if truth(r.get("exact_supported")) and state.startswith("SUPPORTED"):
        return "SUPPORTED"
